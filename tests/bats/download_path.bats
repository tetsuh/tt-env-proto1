...
@test "tt-env install downloads release artifacts when PPA is disabled" {
  ...
  for component in tt-kmd tt-smi firmware tt-metal; do
    cmp -s "${BATS_TEST_TMPDIR}/assets/${component}" "${TT_HOME}/versions/2026.05.16/artifacts/${component}"
  done
...
@test "tt-env install reports missing download metadata for string components" {
  rm -f "${TT_HOME}/releases/2026.05.16.json"
...
@test "tt-env install removes stale partial directories before downloading" {
  mkdir -p "${TT_HOME}/versions/.2026.05.16.partial"
  touch "${TT_HOME}/versions/.2026.05.16.partial/garbage"

  run "$TT_ENV" install 2026.05.16
  [ "$status" -eq 0 ]
  [ ! -e "${TT_HOME}/versions/2026.05.16/garbage" ]
}
