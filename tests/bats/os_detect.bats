...
@test "detect_os reads Linux Mint 22.2 from os-release" {
  os_release="${BATS_TEST_TMPDIR}/os-release"
  printf '%s\n' \
    'NAME="Linux Mint"' \
    'ID=linuxmint' \
...
@test "detect_os honors Linux Mint 22.2 override" {
  missing_os_release="${BATS_TEST_TMPDIR}/missing-os-release"

  run env TT_OVERRIDE_OS_ID=linuxmint TT_OVERRIDE_OS_VERSION=22.2 \
    bash -c 'source "$1"; detect_os "$2"; printf "%s %s\n" "$OS_ID" "$OS_VERSION"' \
    bash "$CORE_SH" "$missing_os_release"
  [ "$status" -eq 0 ]
  [[ "$output" == *"linuxmint 22.2"* ]]
